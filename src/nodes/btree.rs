...
impl<A: BTreeValue> Node<A> {
    ...
    pub(crate) fn lookup_mut<BK>(&mut self, pool: &mut RefPool<Node<A>>, key: &BK) -> Option<&mut A>
    where
        A: Clone,
        BK: Ord + ?Sized,
        A::Key: Borrow<BK>,
        ...
        match A::search_key(&self.keys, key) {
            ...
            Err(index) => match self.children[index] {
                ...
                Some(ref mut child_ref) => {
                    let child = PoolLikeClone::make_mut(pool, child_ref);
        ...
        A::Key: Borrow<BK>,
    {
        ...
        match A::search_key(&self.keys, key) {
            ...
            Err(index) => match self.children[index] {
                ...
                Some(ref mut node) => {
                    PoolLikeClone::make_mut(pool, node).lookup_prev_mut(pool, key)
                }
            },
        }
    }
        ...
        A::Key: Borrow<BK>,
    {
        ...
        match A::search_key(&self.keys, key) {
            ...
            Err(index) => match self.children[index] {
                ...
                Some(ref mut node) => {
                    PoolLikeClone::make_mut(pool, node).lookup_next_mut(pool, key)
                }
            },
        }
    }
    ...
    pub(crate) fn insert(&mut self, pool: &mut RefPool<Node<A>>, value: A) -> Insert<A>
    where
        A: Clone,
    {
        if self.keys.is_empty() {
    ...
    pub(crate) fn remove<BK>(&mut self, pool: &mut RefPool<Node<A>>, key: &BK) -> Remove<A>
    where
        A: Clone,
        BK: Ord + ?Sized,
        A::Key: Borrow<BK>,
        ...
        match action {
            ...
            RemoveAction::StealFromLeft(index) => {
                ...
                let out_value;
                {
                    ...
                    let left = PoolLikeClone::make_mut(pool, children.next().unwrap());
                    let child = PoolLikeClone::make_mut(pool, children.next().unwrap());
            ...
            RemoveAction::StealFromRight(index) => {
                ...
                let out_value;
                {
                    let mut children = self.children.as_mut_slice()[index..index + 2]
                        ...
                        .map(|n| n.as_mut().unwrap());
                    let child = PoolLikeClone::make_mut(pool, children.next().unwrap());
                    let right = PoolLikeClone::make_mut(pool, children.next().unwrap());
                    // Prepare the rebalanced node.
